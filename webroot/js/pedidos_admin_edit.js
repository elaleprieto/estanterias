$(document).ready(function() {
	$('input[value="Quitar"]').click(function() {
		quitarArticulo($(this));
	});
	$('#cantidad_articulos').html(parseInt($('#serial_articulos').html()));
});
/**
	...
	if(articulo_id > 0) {
		...
		if(cantidad && !(isNaN(cantidad))) {
			if(!existeEnLista(articulo_id)) {
				...
				// columna Estado
				inputEstado = $('<input />').attr("class", "pedidos_add").attr("type", "checkbox").attr("name", "data[Orden][" + indice + "][estado]");
				columnaEstado = $('<td>').append(inputEstado);
				...
				// columna para indicar Artículo Sin Cargo o No (oculta)
				if($('#sin_cargo_ckeckbox').attr('checked')) {
					sinCargo = 1;
				}
