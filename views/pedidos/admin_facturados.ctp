...
<div class="pedidos_index">
	<h2><?php __('Pedidos Facturados');?></h2>
	<table cellpadding="0" cellspacing="0">
		<tr>
			...
			<th><?php echo $this -> Paginator -> sort('Facturado', 'Pedido.facturado');?></th>
			<th><?php echo $this -> Paginator -> sort('Tiempo[m]', 'Pedido.tiempo_facturacion');?></th>
		...
		<tr<?php echo $class;?>>
			...
			<td>
				<?php
					...
					$aux = $pedido['Pedido']['facturado'] ? $pedido['Pedido']['facturado'] : $pedido['Pedido']['finalizado'];
					echo $this -> Time -> format($format = 'd/m/Y H:i', $aux);
				?>
			</td>
			<td><?= sprintf("%.1f", $pedido['Pedido']['tiempo_facturacion'] / 60)?></td>
			...
			<td class="actions">
				...
				?>
				<?php echo $this -> Html -> link('Despachado', array(
						'controller' => 'pedidos',
						'action' => 'despachados',
						$pedido['Pedido']['id']
					), null, sprintf('¿Ha sido despachado el pedido de %s?', $pedido['Cliente']['nombre']));
				?>
			</td>
			</tr> <?php endforeach;?>
	</table>
